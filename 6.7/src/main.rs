...
use raylib::{file::File, hittable::Hittables, ray::Ray, sphere::Sphere, vec::Vec3};
...
fn main() {
    ...
    for h in (0..height).rev() {
        ...
        for w in 0..width {
            ...
            let mut color: Vec3;
            let (hit, at) = world.hit(&ray, 0.0, f64::INFINITY);
            if hit {
                color = at.normal.add(&Vec3::new(1.0, 1.0, 1.0)).mul(0.5);
            } else {
                color = ray.color();
            }
            color.x *= 255.999;
